...
let
  ...
  forbidden = [
    ...
    # ★ THE HEADER'S OWN DISTINCTIVE CLAIM, WHICH THE GENERIC LIST ABOVE DOES NOT REACH: gen-product
    # "depends on gen-prelude only and imports no other gen library — its integration with gen-graph /
    # gen-scope / gen-schema / gen-select is the shared accessor-record convention, NOT CODE". A
    # nixpkgs ban says nothing about that, and a sibling import would have passed every cell here.
    # The four names below occur in this repository only in PROSE — the flake description's own
    # sentence, and the headers of `lib/default.nix`, `lib/view.nix`, `lib/factor.nix` — which the
    # strip removes, so a red on one of them is a real import and never a mention. `gen-prelude` is
    # deliberately absent: it is the one dependency the claim allows, and it appears in `flake.nix`
    # as live code.
    "gen-graph"
    "gen-scope"
    "gen-schema"
