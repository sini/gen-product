...
let
    ...
    relations = [
      {
        ...
        pairs = [
          { host = hosts.H_axon01; user = users.U_sini; }
          { host = hosts.H_axon02; user = users.U_vic; }
        ];
      }
    ];
  };
  ...
  joinR = gp.restrict p {
    relations = [
      { dims = [ "host" ]; pairs = [ { host = hosts.H_axon01; } { host = hosts.H_blade01; } ]; }
      { dims = [ "user" ]; pairs = [ { user = users.U_sini; } ]; }
    ];
  };
  filterR = gp.restrict p {
    predicate =
      coords:
      lib.elem coords.host.id_hash [ "H_axon01" "H_blade01" ] && coords.user.id_hash == "U_sini";
  };
  ...
  tp = gp.restrict (gp.productN "cartesian" [ (idFactor "t" throwingGraph) (idFactor "y" fx.gB) ]) {
    predicate = _: true;
  };

  nonMember = builtins.tryEval (gp.cell byCells { host = hosts.H_axon01; user = users.U_vic; });
  ...
  ordered = gp.restrict p {
    cells = [
      { host = hosts.H_blade01; user = users.U_vic; }
      { host = hosts.H_axon01; user = users.U_sini; }
      { host = hosts.H_blade01; user = users.U_vic; } # duplicate
    ];
  };
in
{
  flake.tests.restrict-membership = {
    ...
    test-induced-drops-nonmembers = {
      expr = byCells.edges (gp.cell byCells { host = hosts.H_axon01; user = users.U_sini; });
      expected = [ ];
    };
    ...
    test-join-pinned-order = {
      ...
      expected = [ "H_axon01" "H_blade01" ];
    };
    # restrict∘restrict = intersection.
    test-conjunction = {
      expr = map (c: { h = c.host.id_hash; u = c.user.id_hash; }) (gp.cells conj);
      expected = [ { h = "H_axon01"; u = "U_sini"; } ];
    };
    # predicate-only adjacency never enumerates the throwing-nodes product.
    test-predicate-adjacency-no-enumerate = {
      expr = (builtins.tryEval (builtins.deepSeq (tp.edges (gp.cell tp { t = "t0"; y = "b0"; })) true)).success;
      expected = true;
    };
    # cells list: first-seen dedup, given order preserved (no silent reorder).
    test-cells-order-dedup = {
      expr = map (c: { h = c.host.id_hash; u = c.user.id_hash; }) (gp.cells ordered);
      expected = [
        { h = "H_blade01"; u = "U_vic"; }
        { h = "H_axon01"; u = "U_sini"; }
      ];
