...
in
{
  ...
  flake.tests.entry.test-standalone-entry-matches-lib = {
    ...
    expected = builtins.attrNames genProduct;
  };
  ...
  flake.tests.entry.test-control-the-compared-surface-is-non-trivial = {
    expr = builtins.length (builtins.attrNames standalone) > 10;
    expected = true;
  };
  ...
  flake.tests.entry.test-control-the-comparison-discriminates = {
    expr = builtins.attrNames standalone == builtins.attrNames (removeAttrs genProduct [ "show" ]);
    expected = false;
  };
  ...
  # runs a `prelude` combinator (`concatStringsSep`) inside the library, so a shim forwarding
  # something that is not gen-prelude fails here rather than passing quietly.
  flake.tests.entry.test-the-shims-library-is-live = {
    expr = standalone.show.subset [
      "a"
      "b"
    ];
    expected = "{a,b}";
