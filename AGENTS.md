...
| Minting identity (`id_hash`), kinds, registries                                              | `gen-schema` — "gen-schema: typed record registry with extension points for the pure-gen module system". gen-product only *reads* `id_hash`, as the default factor `key` (`lib/factor.nix:37`) and the default quotient `key` (`lib/quotient.nix:30`) |
...
A bare accessor-graph is also accepted; it gets the identity codec and a positional `dim` (`lib/factor.nix:30-46`).
...
| `coord dim entry` requires `entry ? id_hash` (gen-select `lib/adapters/product.nix:16`) | satisfied on the registry path, where `key` defaults to `entry: entry.id_hash`    | `lib/factor.nix:37`; `(gp.coordsOf pg cid).host.id_hash` ⇒ `"H_a"`                                                     |
...
| Bare accessor-graph factors get **positional** dimension names                                                                                                                                                                                                                                                                                                                                                                                   | `lib/factor.nix:42`; `pgBare.product.dims` ⇒ `["0","1"]`, and `gp.cell pgBare { "0" = "a0"; "1" = "b0"; }` ⇒ `"[\"a0\",\"b0\"]"`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
